...
object RunTestClubs extends App {
...
object MT { // TODO add to razie.Threads

  /** same as forkjoin, but use the given number of threads and slice the iterable */
  def slicejoin[A, B <: Any](t: Int, as: Seq[A])(f: A => B): Seq[Option[B]] = {
    val slice = as.size / t
    ...
    threads.flatMap(_.res).toList // use toList since the iterable may not be strict
  }

  class SliceFuncValThread[A, B <: Any](r: Range, val as: Seq[A], val f: A => B) extends java.lang.Thread {
    var res = new ListBuffer[Option[B]]()
