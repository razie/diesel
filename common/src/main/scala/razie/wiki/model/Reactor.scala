...
//    }
  ...
  WikiObservers mini {
    case WikiEvent(_, "WikiEntry", _, Some(x), _, _) if fallbackProps.we.exists(_.uwid == x.asInstanceOf[WikiEntry].uwid) => {
...
class Reactor (val realm:String, val fallBacks:List[Reactor] = Nil, val we:Option[WikiEntry]) {
  ...
  val supers : Array[String] = {
    if(realm == Reactors.RK) Array(Reactors.RK)
...
//          dflt
  ...
  WikiObservers mini {
    case WikiEvent(_, "WikiEntry", _, Some(x), _, _) if props.we.exists(_.uwid == x.asInstanceOf[WikiEntry].uwid) => {
