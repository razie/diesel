    ...
    case hidden ~ stype ~ _ ~ name ~ sig ~ _ ~ lines => {
      ...
      lazyt { (current, ctx) =>
        ...
        hidden.map(x => StrAstNode.EMPTY) getOrElse {
          ...
          // hide it
            // todo why am i hiding it?
            TriAstNode(s"""`{{$stype $name:$signature}}` (<small><a href="javascript:weShowSection('${ctx.we.get.wid.wpath}#$name')">view</a></small>)<br>""", "", "").fold(ctx)
...
//  }
  ...
  //======================= dates
  import razie.tconf.parser.SpecParserSettings.{mth1, mth2}
