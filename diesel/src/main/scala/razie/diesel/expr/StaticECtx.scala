...
//    base.map(_.put(p))
  ...
  override def remove(name: String): Option[P] = {
    throw new DieselExprException("CAN'T OVERWRITE STATIC CTX VARS [remove]")
...
class OLDStaticECtx(cur: List[P] = Nil, base: Option[ECtx] = None, curNode: Option[DomAst] = None)
    extends OSLDSimpleECtx(
      cur, base, curNode) {
