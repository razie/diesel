...
class RazControllerBase extends Controller with Logging with Validation { 
  ...
  /** result a page to display this message and optionally continue to a page */
  def Msg3(msg: String, page: Option[String], pageNO:Option[String], u: Option[WikiUser] = None)(implicit request: Request[_]): play.api.mvc.SimpleResult[play.api.templates.Html] = {
...
trait ViewService {
  def utilMsg (msg:String, link:Option[String], user:Option[WikiUser], link2:Option[String]=None)(implicit request: Request[_]): play.api.mvc.SimpleResult[play.api.templates.Html]
}
