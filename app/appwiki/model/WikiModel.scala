...
case class WikiEntry(
  ...
  def create = {
    ...
    Audit.logdbWithLink (
      AUDIT_WIKI_CREATED, 
      s"/wiki/${wid.wpath}",
      "BY " + (WikiUsers.impl.findUserById(this.by).map(_.userName).getOrElse(this.by.toString)) + " " + category + ":" + name, "\nCONTENT:\n" + this)
  ...
  def delete(sby: String) = {
    ...
    WikiEntryOld(this).create
    val key = Map("category" -> category, "name" -> name, "parent" -> parent)
    Wikis.weTable(wid.cat).m.remove(key)
...
case class WikiEntryOld(entry: WikiEntry, _id: ObjectId = new ObjectId()) {
  def create = RCreate.noAudit[WikiEntryOld](this)
}
...
case class WikiLink(
  ...
  def create = RCreate[WikiLink](this)
