...
object Tribe extends RazController with Logging {
  ...
  def doeClubAddKid(tid: String, kid:String, x:String, role:String) = Action { implicit request =>
    ...
    (for (
      au <- activeUser;
      isClub <- au.isClub orErr ("tribes only for a club");
      tribe <- Tribes.findById(new ObjectId(tid)) orErr ("tribe not found");
      rk <- RacerKidz.findById(new ObjectId(kid)) orErr ("RacerKid not found");
      wid <- Wikis.findById(tid).map(_.wid) orErr ("wiki not found")
    ) yield {
