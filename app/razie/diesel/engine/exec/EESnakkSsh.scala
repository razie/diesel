...
//    }
  ...
  override def apply(in: EMsg, destSpec: Option[EMsg])(implicit ctx: ECtx): List[Any] = {
    ...
    if (in.ea == "snakk.ffd")
      ...
      return sftpDelete(in, destSpec)
    else if (in.ea == "snakk.sftp.list")
      return sftpList(in, destSpec)
    else if (in.ea == "snakk.ssh")
      return ssh(in, destSpec, "exec")
    else if (in.ea == "snakk.shell")
      return ssh(in, destSpec, "shell")
...
//      if(n.length > 1) ctx.get(n) else None
  ...
  def sftpDelete(in: EMsg, destSpec: Option[EMsg])(implicit ctx: ECtx): List[Any] = {
    val host = ctx.getRequired("host")
    val port = ctx.get("port").getOrElse("22")
    val user = ctx.getRequired("user")
    val pwd = decpass(ctx.getRequired("pwd"))
  ...
  def sftpList(in: EMsg, destSpec: Option[EMsg])(implicit ctx: ECtx): List[Any] = {
    val host = ctx.getRequired("host")
    val port = ctx.get("port").getOrElse("22")
    val user = ctx.getRequired("user")
    val pwd = decpass(ctx.getRequired("pwd"))
    ...
    val res = new InfoAccumulator()
    val output = SftpClientBridge.listFiles(remotePath, host, port, user, pwd)
    res += EVal(new P(PAYLOAD, output)) :: Nil
    res.eres
  }

  def ssh(in: EMsg, destSpec: Option[EMsg], chan:String)(implicit ctx: ECtx): List[Any] = {
    val host = ctx.getRequired("host")
    val port = ctx.get("port").getOrElse("22")
    val user = ctx.getRequired("user")
    val pwd = decpass(ctx.getRequired("pwd"))
    ...
    val res = new InfoAccumulator()
    val output = SshClient.execute(cmd, new SshClient.Authorization(host, user, pwd), "exec")
    res += EVal(new P(PAYLOAD, output)) :: Nil
    res.eres
    }

  def shell(in: EMsg, destSpec: Option[EMsg], chan:String)(implicit ctx: ECtx): List[Any] = {
    val host = ctx.getRequired("host")
    val port = ctx.get("port").getOrElse("22")
    val user = ctx.getRequired("user")
    val pwd = decpass(ctx.getRequired("pwd"))
    ...
    val res = new InfoAccumulator()
    val output = SshClient.execute(cmd, new SshClient.Authorization(host, user, pwd), "shell")
    res += EVal(new P(PAYLOAD, output)) :: Nil
    res.eres
  }
