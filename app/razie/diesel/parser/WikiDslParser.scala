...
trait WikiDslParser extends ParserBase {
  ...
  def wikiPropJsFiddle: PS = "{{" ~> """jsfiddle""".r ~ opt(":" ~ rep(arg <~ opt(","))) ~ "}}" ~ opt(
    CRLF1 | CRLF3 | CRLF2) ~ slines <~ ("{{/jsfiddle}}" | "{{/}}") ^^ {
  ...
  def wikiPropFiddle: PS = "{{" ~> """fiddle""".r ~ "[: ]".r ~ """[^:}]*""".r ~ opt(
    ":" ~ rep(arg <~ opt(","))) ~ "}}" ~ opt(CRLF1 | CRLF3 | CRLF2) ~ slines <~ "{{/fiddle}}" ^^ {
      ...
      try {
        lang match {
          case "js" | "html" | "css" => {
            ...
            val hh = (if (ss contains "<head>") rehh.findFirstMatchIn(ss).get.group(1) else "").replaceAll("<script",
              "<scrRAZipt").replaceAll("</script", "</scrRAZipt")
...
//            SState(views.html.fiddle.inlineHtmlfiddle(name, args, ("", "", "", trim("document.write(function(){
//            return " + ss.replaceFirst("\n", "") + "}())")), None).body)
  ...
  //  def dotPropCodeScala: PS = """^\.scala""".r ~ opt(CRLF1 | CRLF3 | CRLF2) ~> lines <~ """^\./""".r ~ opt
  //  ("scala") ^^ {
  //    case lines => {
  ...
  def wikiPropDsl: PS = "{{" ~> opt(".") ~ """dsl\.\w*""".r ~ opt("[: ]".r ~ """[^:}]*""".r) ~ "}}" ~ opt(
    CRLF1 | CRLF3 | CRLF2) ~ slines <~ ("{{/" ~ """dsl\.\w*""".r ~ "}}") ^^ {
    case hidden ~ stype ~ opt ~ _ ~ _ ~ lines => {
      ...
      if (hidden.isDefined) StrAstNode("")
        ...
        StrAstNode(s"""<div><b><small>DSL ${
          stype.replaceFirst("dsl.", "")
        }</b> ($name):</small><br>$fid}</div>""")//, Map.empty, List.empty, List(wffiddle))
      }
    }
  }
}
