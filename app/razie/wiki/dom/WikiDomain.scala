...
/** encapsulates the knowledge to use the wiki-defined domain model
  ...
  */
class WikiDomain (val realm:String, val wi:WikiInst) extends WikiDomainT {
...
object WikiDomain {//extends WikiDomain (Wikis.RK) {
  ...
  WikiObservers mini {
    case e@WikiEvent(_, "WikiEntry", _, Some(x), _, _, _) if x.asInstanceOf[WikiEntry].category == "DslDomain" && x.asInstanceOf[WikiEntry].category == "Category" => {
